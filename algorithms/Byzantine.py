# encoding: utf-8
# Byzantine Generals' Problem is defined as a situation where spread out units need to coordinate 
# their behavior or action but cannot trust each other to get organized.
import threading
import random
import sys
...
class general(threading.Thread):
	...
	def vote(self,path,msg,m):
		# general that was or is not a commandar
...
if __name__ == '__main__':
	...
	# validation check: all loyal obey same orders
	print("Votes: ", loyal_votes)
	for v in loyal_votes:	
		assert(v == loyal_votes[0])

	# if general loyal, loyals obey
